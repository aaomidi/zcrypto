...
func TestParseRevocationList(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("error when parsing CRL: %v", err)
	}
	if !strings.Contains(err.Error(), "x509: malformed GeneralizedTime"){
		t.Fatal("err.Error() did not contain x509: malformed GeneralizedTime")
	}
	...
	_, err = ParseRevocationList(derBytes)
	if !strings.Contains(err.Error(), "x509: malformed UTCTime"){
		t.Fatal("err.Error() did not contain x509: malformed UTCTime")
	}
}
